...
function showView(viewName) {
    ...
    // Manage body classes for special views
...
function initializeBottomNavigation() {
    ...
    if (bottomNavItems.length === 0) {
        console.log('📱 Bottom navigation not found, skipping initialization');
        return;
    }
    ...
    const navMapping = {
        ...
        'bollywood': () => {
            console.log('📱 Bottom nav: Bollywood clicked');
            if (window.BollywoodModule) {
                BollywoodModule.loadBollywoodPage();
            } else {
                showToast('Bollywood module not loaded. Please refresh the page.', 'error', 3000);
        ...
        'tvshows': () => {
            console.log('📱 Bottom nav: TV Shows clicked');
            try {
                loadTVShowsPage();
            } catch (error) {
                console.error('❌ Error loading TV Shows:', error);
        ...
        'movies': () => {
            console.log('📱 Bottom nav: Movies clicked');
            loadMoviesPage();
        },
        'explore': () => {
            console.log('📱 Bottom nav: Explore clicked');
            loadExplorePage();
        },
        'livetv': () => {
            console.log('📱 Bottom nav: Live TV clicked');
            loadLiveTVPage();
        },
        'updates': () => {
            console.log('📱 Bottom nav: Updates clicked');
            loadNewUpdatesPage();
        },
        'history': () => {
    ...
    bottomNavItems.forEach(item => {
        ...
        const handler = navMapping[navType];
        ...
        if (handler) {
            item.addEventListener('click', (e) => {
                ...
                // Sync with desktop navigation if exists
                syncDesktopNavigation(navType);
            });
        }
    });
...
// Sync bottom navigation with desktop navigation
function syncDesktopNavigation(navType) {
    const desktopNavMapping = {
        'home': 'homeBtn',
        ...
        'explore': 'exploreBtn',
        'livetv': 'liveTVBtn',
        'updates': 'newUpdatesBtn',
        'history': 'historyBtn'
    };
    
    const desktopBtnId = desktopNavMapping[navType];
    if (desktopBtnId) {
        ...
        // Update mobile navigation active state (if exists)
        const mobileBtn = document.getElementById(`mobile${desktopBtnId.charAt(0).toUpperCase() + desktopBtnId.slice(1)}`);
        if (mobileBtn) {
            document.querySelectorAll('.mobile-nav-links .nav-link').forEach(btn => btn.classList.remove('active'));
            mobileBtn.classList.add('active');
        }
    }
}

// Update bottom navigation active state from other navigation methods
function updateBottomNavigation(viewName) {
    const bottomNavItems = document.querySelectorAll('.bottom-nav-item');
    const navMapping = {
        'home': 'home',
        'bollywood': 'bollywood',
    ...
    const bottomNavType = navMapping[viewName];
    if (bottomNavType) {
        bottomNavItems.forEach(item => {
            item.classList.remove('active');
            if (item.getAttribute('data-nav') === bottomNavType) {
                item.classList.add('active');
            }
        });
    }
